...
## 📝 Your Notes
- difference between .catch and .then. .catch would error report on the fetchPokemon promise and the setPokemon call. .then would just error report on the setPokemon call
- you can use error boundaries to reset the state of your app. in this case, reset keys allows us to reset the pokemonName input, allowing users to continue using the app with little interruption to flow.
