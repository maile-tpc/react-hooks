...
- lifting state is to move state management to the lowest parent. this way, we can share state between sibling components
- colocate state to where it needs to be (move things to where they live, the closest it can go)
## Background
